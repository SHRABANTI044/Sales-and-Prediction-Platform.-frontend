...
const Dashboard = ({ onNavigate, onLogout }) => {
  ...
  const chartData = useMemo(() => {
    ...
    switch (selectedPeriod) {
      case '7 days':
        ...
        dataPoints = periods.map((day, index) => ({
          period: day,
          actual: Math.round(baseValue * (0.12 + (index * 0.02))),
          predicted: Math.round(baseValue * (0.13 + (index * 0.025)))
        }));
        break;
      case '30 days':
        ...
        dataPoints = periods.map((week, index) => ({
          period: week,
          actual: Math.round(baseValue * (0.22 + (index * 0.03))),
          predicted: Math.round(baseValue * (0.24 + (index * 0.035)))
        }));
        break;
      case '90 days':
        periods = ['Jan', 'Feb', 'Mar'];
        dataPoints = periods.map((month, index) => ({
          period: month,
          actual: Math.round(baseValue * (0.7 + (index * 0.15))),
          predicted: Math.round(baseValue * (0.75 + (index * 0.18)))
        }));
        break;
      default:
        periods = ['Week 1', 'Week 2', 'Week 3', 'Week 4'];
    ...
    return dataPoints;
  }, [selectedPeriod, dashboardData.totalSales]);

  // Calculate max value for scaling
  const maxValue = useMemo(() => {
    if (!chartData || chartData.length === 0) return 50000;
    return Math.max(...chartData.map(point => Math.max(point.actual, point.predicted)));
  }, [chartData]);
  ...
  return (
    <div className="dashboard-container">
      ...
      {/* Sidebar */}
      <aside className={`dashboard-sidebar ${isMobileSidebarOpen ? 'mobile-open' : ''}`} onClick={() => setIsMobileSidebarOpen(false)}>
          ...
          <div className="analytics-row">
            ...
            <div className="chart-container full-width">
              ...
              <div className="chart-area">
                <svg className="chart-svg" width="100%" height="100%" viewBox="0 0 500 250" preserveAspectRatio="xMidYMid meet">
                  <defs>
                  ...
                  {/* Actual Sales Area */}
                  <path
                    d={`M 80,${200 - (safeChartData[0]?.actual / maxValue * 150)} ${safeChartData.map((point, index) => {
                      const x = 80 + index * (320 / Math.max(1, safeChartData.length - 1));
                      const y = 200 - (point.actual / maxValue * 150);
                      return `L ${x},${y}`;
                  ...
                  <path
                    d={`M 80,${200 - (safeChartData[0]?.predicted / maxValue * 150)} ${safeChartData.map((point, index) => {
                      const x = 80 + index * (320 / Math.max(1, safeChartData.length - 1));
                      const y = 200 - (point.predicted / maxValue * 150);
                      return `L ${x},${y}`;
                    }).join(' ')} L ${80 + (safeChartData.length - 1) * (320 / Math.max(1, safeChartData.length - 1))},200 L 80,200 Z`}
                  ...
                  {safeChartData.map((point, index) => {
                    const x = 80 + index * (320 / Math.max(1, safeChartData.length - 1));
                    const actualY = 200 - (point.actual / maxValue * 150);
                    const predictedY = 200 - (point.predicted / maxValue * 150);
          ...
          <div className="bottom-row">
            ...
            <div className="motivational-tips-container">
              ...
              <div className="tips-content">
                ...
                <p className="tip-text">{motivationalTips[currentTipIndex]}</p>
