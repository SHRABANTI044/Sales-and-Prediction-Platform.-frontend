...
const BusinessSetup = ({ onComplete }) => {
  ...
  const isFormComplete = () => {
    return (
      ...
      (uploadData.selectedFile || uploadData.showSampleData)
    );
  };
...
Potato Bag (3 KG),70,40,100,Food & Beverages,2024-01-15`;
  ...
  const handleGoToDashboard = async () => {
    // Validate all required business data
    ...
    // Check if user has either uploaded a file or wants to use sample data
    if (!uploadData.selectedFile && !uploadData.showSampleData) {
      alert('Please either upload your CSV file or use sample data to continue');
      return;
    }
    ...
    <div className="business-setup-container">
      <div className="business-setup-card">
        ...
        <div className="action-section">
          <button 
            className={`dashboard-btn ${isLoading ? 'loading' : ''} ${!isFormComplete() ? 'disabled' : ''}`}
          ...
          >
            {isLoading ? (
              ...
              <>
                ...
                {isFormComplete() ? 'Go to Dashboard' : 'Complete all fields to continue'}
              </>
            )}
          </button>
        </div>
